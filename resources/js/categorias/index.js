...
const state = {
    ...
    categoriasTableContainer: document.getElementById('table-container'),
    categoriasForm: document.getElementById('categoriaForm'),
    categoriasFieldContainer: document.getElementById('categoriaFields'),
    modalOpenButtons: document.getElementsByClassName('categoriaModalBtn'),
    ...
    deleteCategoriaButton: document.getElementsByClassName('js-delete-categoria'),
...
async function fetch_rows() {
    try {
        ...
        state.categoriasTableContainer.innerHTML = rowsHTML;
        ...
        state.isTableLoading = false;
        state.categoriasTableContainer.innerHTML = `
...
async function bind_modal_events() {
    ...
    for (let modalOpenButton of state.modalOpenButtons) {
        modalOpenButton.addEventListener('click', async function() {
            ...
            await load_categoria_fields(id);
            state.categoriaModal.modal('show');
            state.categoriasForm.onsubmit = async (e) => {
                e.preventDefault();
                state.validateForm();
                ...
                if (!responseJson.error) {
                    state.categoriaModal.modal('hide');
                    await fetch_rows();
                    createToast('Administración de Categorías',
                        `Se guardó correctamente la información.
                         <a href="${state.WEB_URL}/${responseJson.data}"
                        class="btn btn-success btn-sm">Ver</a>`, true);
                } else {
                    if (isEmpty(responseJson.error)) {
                        createToast('Administración de Categorías', responseJson.error, false);
                    }
                    else {
                        // is validation error
                        for (const key in responseJson.error) {
                            createToast('Administración de Categorías', responseJson.error[key], false);
                        }
                    }
                }
            }
    ...
    for (let deleteButton of state.deleteCategoriaButton) {
        deleteButton.addEventListener('click', async function() {
            ...
            if (confirmResponse) {
                ...
                console.log(responseJson);
                if (!responseJson.error) {
                    createToast('Administración de Categorías', 'Se eliminó correctamente la información', true);
                    await fetch_rows();
                } else {
                    if (isEmpty(responseJson.error)) {
                        createToast('Administración de Categorías', responseJson.error, false);
...
async function load_categoria_fields(id) {
    let url = state.WEB_URL + '/get_categoria_fields';
    if (id) {
        url += '?id=' + id;
    }
    ...
    state.categoriasFieldContainer.innerHTML = fieldsHTML;
}
